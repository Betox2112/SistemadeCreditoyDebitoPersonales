...
public class Sistema
{
    ...
    private Log log = Log.GetInstance("log.txt"); // Centralizar aquí
            ...
            switch (opcion)
            {
                case "1":
                    ...
                    else
                    {
                        ...
                        log.RegistrarAccion(usuarioActual, $"Crédito registrado: {nuevoCredito.Nombre} para cliente {cedulaCredito}");
                        Console.WriteLine();
                        Console.WriteLine("------------------------------------");
            ...
            switch (opcion)
            {
                case "1":
                    ...
                    else
                    {
                        ...
                        log.RegistrarAccion(usuarioActual, $"Débito registrado: {descripcion} para cliente {cedulaDebito}");
    ...
    private void GuardarEnArchivo()
    {
        ...
        try
        {
            ...
            log.RegistrarAccion(usuarioActual, "Clientes guardados en archivo");
            Console.WriteLine("  Clientes guardados en el archivo correctamente.");
        }
        catch (Exception ex)
        {
            log.RegistrarAccion(usuarioActual, $"Error al guardar en archivo: {ex.Message}");
    ...
    private void CargarDesdeArchivo()
    {
        ...
        if (File.Exists("clientes.txt"))
        {
            ...
            foreach (var linea in lineas)
            {
                ...
                {
                    log.RegistrarAccion(usuarioActual, "Error al cargar desde archivo: Línea con datos incompletos");
                    Console.WriteLine("  Error: Línea con datos incompletos.");
                }
            }
            log.RegistrarAccion(usuarioActual, "Datos cargados desde archivo correctamente");
